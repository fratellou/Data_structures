...
public:
    Object(T initial_data, string object_name) : data(initial_data), name(object_name) {}
...
public:
    User(const string& uname, const string& pass, int clearanceLevel) 
        : username(uname), password(pass), securityClearanceLevel(clearanceLevel) {}
    ...
    void changePassword(const string& oldPass, const string& newPass) {
        ...
        else cout << "Повторите попытку ввода старого пароля.\n";
    }
    ...
    void userProfile() const {
        cout << "Логин пользователя: " << username << ", Уровень доступа: " << securityClearanceLevel << endl;
    }
};
...
public:
    ...
    void renameFile(const string& oldname, const string& newname) {
        if (rename(oldname.c_str(), newname.c_str()) == 0) {
            cout << "Файл успешно переименован из \"" << oldname << "\" в \"" << newname << "\".\n";
        } else {
