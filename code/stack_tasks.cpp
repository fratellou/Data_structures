...
void task1() {
    ...
    cout << "Результат вычисления: " << calculatePostfix(postfixExpression) << endl << endl;
}
...
string infixToPostfix(const string& expression) {
    ...
    for (char c : expression) {
        if (isdigit(c))
            ...
            // Extract the operators from the stack and add them to the postfix expression until the opening parenthesis is encountered
            ...
            operators.pop(); 
             ...
             // Extracting operators from the stack while the priority of the current operator is less than or equal to the priority of the operator at the top of the stack
            while (operators.size != 0 && precedence(c) <= precedence(operators.head->data)) {
                postfix += operators.head->data;
                operators.pop();
            }
            operators.push(c);
...
int calculatePostfix(const string& postfix) {
    ...
    for (char c : postfix) {
        if (isdigit(c))
            ...
            switch (c) {
                case '+': operands.push((operand1 + operand2) + '0'); break;
                case '-': operands.push((operand1 - operand2) + '0'); break;
                case '*': operands.push((operand1 * operand2) + '0'); break;
                case '/': operands.push((operand1 / operand2) + '0'); break;
            }
...
int isBalanced(const string& sequence) {
    ...
    // If the stack is empty after processing the string, the brackets are balanced
    if (brackets.size == 0) {
        return 1;
    }
    return 0;
