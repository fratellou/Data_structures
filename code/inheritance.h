...
class Federal_Security_Servicee : public Employee {
 ...
 public:
  ...
  void doWork() const {
    cout << "Была осуществлена деятельность, направленная на защиту "
            "разведывательной программы Агентства от разведывательной службы "
            "оппозиции - "
         << name << '-' << employeeID << endl;
  }

  void setSecurityLevel(const int& level) { securityLevel = level; }
...
class The_Office_of_the_National_AntiTerrorism_Committee : public Employee {
 ...
 public:
  The_Office_of_the_National_AntiTerrorism_Committee(const string& empName,
                                                     int empID)
      : Employee(empName, empID) {}

  void doWork() const {
    cout << "Были произведены меры по противодействию терроризму - " << name
         << '-' << employeeID << endl;
  }

  int getNumberOfPersonnel() const { return numberOfNACPersonnel; }
};

// Subclass "Сотрудник Службы по защите конституционного строя и борьбе с
// терроризмом"
class
    Service_for_the_Protection_of_the_Constitutional_Order_and_the_Fight_against_Terrorism
    : public Employee {
 private:
 ...
 public:
  Service_for_the_Protection_of_the_Constitutional_Order_and_the_Fight_against_Terrorism(
      const string& empName, int empID)
      : Employee(empName, empID) {}

  void doWork() const {
    cout << "Были произведены меры по предотвращению внутриполитических угроз "
