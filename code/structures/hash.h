...
public:
    ...
    HashTableNode(const string& k, const string& val) : key(k), element(val), next(nullptr) {}
};
...
public:
    HashTable(int initialCapacity = 100, float loadFactor = 0.75) : capacity(initialCapacity), size(0), loadThreshold(loadFactor) {
