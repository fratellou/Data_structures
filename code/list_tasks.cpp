...
int main() {
  ...
  government_service.add("Разведка");
  government_service.add("Пограничная-деятельность"); // adding a new node with a value to the beginning of the linked list
  cout << "2. Список после добавления элементов в начало:\n"; 
  government_service.print();

  government_service.addToEnd("Обеспечение-информационной-безопасности");
  government_service.addToEnd("Кадровый-состав");   // adding a new node with a value to the end of the linked list
  cout << "\n3. Список после добавления элементов в конец:\n"; 
  government_service.print();

  cout << "\n4. Результат проверки наличия первого узла связанного списка: " << testRemoveFirstNode(&government_service); // checking for the presence and deleting the first node from the linked list
  cout << "Список после проверки наличия и удаления первого узла из связанного списка:\n"; 
  government_service.print();

  cout << "\n5. Результат проверки наличия и удаления всех узлов со значением 'Разведка' из связанного списка: " << testRemoveValues(&government_service, "Разведка"); // checking for the presence and deleting the first node from the linked list
  cout << "Список после проверки наличия и удаления всех узлов со значением 'Разведка' из связанного списка:\n"; 
  government_service.print();
...
// checking for the presence and removal of all nodes with the value from the linked list
string testRemoveValues(LinkedList* myList, const string value) {
    ...
    if ((*myList).head != NULL) {
        ...
        assert(search_result != -1); // Checking that the value of the deleted element matches the expected value
